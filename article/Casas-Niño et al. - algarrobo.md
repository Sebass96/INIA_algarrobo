...
![Sensitivity of phenotypic differentiation ($P_{ST}$) to the ratio$c/h^{2}$ for nine quantitative traits in *Neltuma pallida*. Solid red lines represent the estimated $P_{ST}$​ values and dashed red lines indicate the 95% bootstrap confidence intervals obtained from 1,000 resamples. The green dashed line denotes the neutral genetic differentiation ($F_{ST} = 0.03$) reported for the species (Castillo et al., 2021). Traits include growth (dbh, crown diameter, branch height, total height), stem and architectural quality (tree form, trunk quality), and reproductive or foliar traits (fruit production, fruit quality, foliage quality).](img_4.jpg){#fig:id.xpg8j1my0n1x}
