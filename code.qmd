...
rslt <- 7:length(mgroup) %>% map(\(a) {
  ...
  var <- match("DAP", names(h2d))
  
  groups <- var:length(h2d) %>% map(\(b) {
    ...
    h2d %>%  
       H2cal(data = .
        ...
        , gen.name = "Cuenca"
        , rep.n = 1
        , fixed.model = "0 + (1|Departamento) + Cuenca"
        , random.model = "1 + (1|Departamento) + (1|Cuenca)"
        , emmeans = FALSE
        , plot_diag = TRUE
        , outliers.rm = TRUE
        , trial = npop
...
h2tab <- 1:length(rslt) %>% map(\(x) {
  ...
  bind_rows() %>% 
  rename(Trait = trait, Heritability = H2.c, Population = trial) %>% 
