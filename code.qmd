...
# descriptivas %>%
#   write_sheet(ss = gs, sheet = "Environmental_descrip")
...
mv <- data_variables %>%
  ...
  select(-`Forking`, -`Branch height`, -Station,-Basin) %>% 
  group_by(Locality) %>%
  summarise(across(DBH:`Foliage quality`, ~ mean(.x, na.rm = TRUE)))
...
grupos_ind <- as.factor(datos$Locality)
...
pt <- fviz_pca_biplot(pca,
  ...
  col.ind = grupos_ind,
  # palette = c(
  #   "Cuantitative" = "steelblue",
  #   "Cualitative" = "forestgreen"
  # ),
  legend.title = "Variables type",
  geom.var = c("arrow", "text"),
  repel = TRUE,
...
pt %>%
  ggsave2(plot = ., "submission/Figure_x.jpg", height = 8, width = 10, units = "in", dpi = 300)
