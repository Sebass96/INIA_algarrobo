...
datos <- gs %>%
  ...
  select(
    ID = IDENTF, Depart, Estación,
    Num_ramas_XX:Parasitos_XX, Localidad:Suelo,
    Forma_leg:Bif_leg
  ) %>%
  select(
    ID, Depart, Provincia, Distrito, Localidad, Estación,
    DAP:Altura_total, Num_ramas_XX,
    Forma_arbol, Forma_leg,
    Calidad_tronco, Cal_tronq_leg,
    Produccion_frutos, Prod_fru_leg,
    Calidad_frutos, Cal_fru_leg,
    Calidad_follaje, Cal_foll_leg,
    Bifurcacion, Bif_leg,
    `fito_IIhEd _k`:Parasitos_XX, Carbonatos:Suelo
  ) %>%
  mutate(across(Forma_arbol:Parasitos_XX, as.factor))
